...
from src.config import INTERIM_DATA_DIR, LOGS_DIR, ClimateParams
...
class PowerGridProcessor:
    ...
    def _build_graph(self):
        ...
        # 加载图结构
        g = ig.Graph.Read_Pickle(self.model_path)
        # 读取坐标经纬度
    ...
    def _compute_hodge_laplacian(self, g):
        ...
        # 归一化邻接矩阵 计算 D^(-1/2) * A * D^(-1/2)，先提取对角元素，计算 -0.5 次方，再构建对角矩阵
...
def main(
    ...
    processor = PowerGridProcessor(model_path=model_path, output_dir=output_dir)
